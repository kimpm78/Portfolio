...
import { ArrowLeft, Eye, EyeOff, Link, Pencil, Plus, Save, Trash2, Upload, X } from 'lucide-vue-next'
...
const importLegacyWorks = async () => {
  const confirmed = window.confirm('既存のWORKSデータをCloudinaryへアップロードし、Firestoreへ登録しますか？')
...
const uploadSelectedImages = async () => {
  ...
  if (selectedMainImageFile.value) {
    const result = await uploadImageToCloudinary(selectedMainImageFile.value, 'portfolio/works/main')
    imageUrl = result.url
  ...
  <main class="min-h-screen bg-[rgb(27,29,32)] px-6 py-8 text-white">
    <div class="mx-auto max-w-7xl">
      ...
      <section class="mt-8 grid gap-8 xl:grid-cols-[460px_1fr]">
        <form class="rounded-[2rem] border border-white/10 bg-black/30 p-6 shadow-[0_18px_50px_rgba(0,0,0,0.28)]" @submit.prevent="saveWork">
          ...
          <div class="space-y-5">
            ...
            <div class="block">
              ...
              <div class="grid gap-3">
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">GitHub</span>
                  <input
                ...
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Figma</span>
                  <input
                ...
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Demo</span>
                  <input
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
              <label class="block">
                ...
                <input type="file" accept="image/*" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white/70 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:font-bold file:text-slate-950" @change="setMainImageFile" />
              ...
              <label class="block">
                <span class="mb-2 block text-sm font-bold text-white/60">ギャラリー画像（最大：4枚）</span>
                <input type="file" accept="image/*" multiple class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white/70 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:font-bold file:text-slate-950" @change="setGalleryFiles" />
            ...
            <div class="block">
              <span class="mb-2 block text-sm font-bold text-white/60">ギャラリー画像URL（最大：4枚）</span>
              <div class="grid gap-3">
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Gallery 1</span>
                  <input
                ...
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Gallery 2</span>
                  <input
                ...
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Gallery 3</span>
                  <input
                ...
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-white/35">Gallery 4</span>
                  <input
        ...
        <section>
          <div class="mb-5 flex items-center justify-between gap-4">
            ...
            <button type="button" class="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm font-bold text-white/60 transition hover:text-white" @click="loadWorks">
          ...
          <div v-else class="grid gap-5 md:grid-cols-2">
            ...
            >
              <div class="relative h-52 bg-black/40">
                <img v-if="item.imageUrl" :src="item.imageUrl" :alt="item.imageAlt" class="h-full w-full object-cover" />
                ...
                <div class="absolute left-5 top-5 flex gap-2">
                  ...
                  <span v-if="item.isMobileImage" class="rounded-full bg-[rgb(255,255,130)] px-3 py-1.5 text-xs font-black text-slate-950">
                    Mobile
                  </span>
                  <span v-if="item.imageUrls.length" class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold text-white/70 backdrop-blur">
                    +{{ item.imageUrls.length }} Gallery
                  </span>
                </div>
                ...
                <div class="absolute bottom-5 left-5 right-5">
                  <p class="text-xs font-bold uppercase tracking-[0.18em] text-[rgb(255,255,130)]/80">
              ...
              <div class="p-5">
                ...
                <ul class="mt-4 flex flex-wrap gap-2">
                  <li v-for="tag in item.tags" :key="`${item.id}-${tag}`" class="rounded-full bg-white/5 px-3 py-1.5 text-xs font-bold text-white/60">
                    {{ tag }}
                  </li>
                </ul>

                <div v-if="item.links.length" class="mt-4 flex flex-wrap gap-2 text-xs font-bold text-[rgb(255,255,130)]/80">
                  <span v-for="link in item.links" :key="`${item.id}-${link.url}`" class="inline-flex items-center gap-1.5">
                ...
                <div class="mt-5 flex gap-2">
                  <button type="button" class="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-black text-slate-950 transition hover:bg-[rgb(255,255,130)]" @click="editWork(item)">
                  ...
                  <button type="button" class="inline-flex items-center justify-center rounded-full border border-red-300/20 px-4 py-2 text-red-200 transition hover:bg-red-500/10" @click="deleteWork(item.id)">
