...
const importLegacyDesignGraphics = async () => {
  const confirmed = window.confirm('既存のDESIGN & GRAPHICデータをCloudinaryへアップロードし、Firestoreへ登録しますか？')
  ...
  try {
    ...
    for (const [index, work] of staticDesignGraphicWorks.entries()) {
      ...
      const imageUrl = await uploadLegacyImage(work.image, `portfolio/design-graphic/legacy/${slug}`)
      const logoUrl = work.logo ? await uploadLegacyImage(work.logo, `portfolio/design-graphic/legacy/${slug}/logos`) : ''
...
const uploadSelectedFiles = async () => {
  ...
  if (selectedImageFile.value) {
    const result = await uploadImageToCloudinary(selectedImageFile.value, 'portfolio/design-graphic')
    imageUrl = result.url
  }

  if (selectedLogoFile.value) {
    const result = await uploadImageToCloudinary(selectedLogoFile.value, 'portfolio/design-graphic/logos')
    logoUrl = result.url
  }
...
<template>
  <main class="min-h-screen bg-[rgb(27,29,32)] px-6 py-8 text-white">
    <div class="mx-auto max-w-7xl">
      ...
      <section class="mt-8 grid gap-8 xl:grid-cols-[440px_1fr]">
        <form class="rounded-[2rem] border border-white/10 bg-black/30 p-6 shadow-[0_18px_50px_rgba(0,0,0,0.28)]" @submit.prevent="saveDesignGraphic">
          ...
          <div class="space-y-5">
            <label class="block">
              ...
              <input v-model="form.title" type="text" placeholder="例：KAZARU PROJECT" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
              <label class="block">
                ...
                <input v-model="form.category" type="text" placeholder="例：Brand Design" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
                <p class="mt-2 text-xs leading-5 text-white/35">
                  作品の種類を短く入力します。
                </p>
              </label>
              <label class="block">
                ...
                <input v-model="form.period" type="text" placeholder="例：2026" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
                <p class="mt-2 text-xs leading-5 text-white/35">
                  制作年や制作期間を表示します。
                </p>
              </label>
            ...
            <label class="block">
              ...
              <textarea v-model="form.description" rows="4" placeholder="制作意図、担当範囲、ビジュアルの特徴を入力してください。" class="w-full resize-none rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40"></textarea>
              <p class="mt-2 text-xs leading-5 text-white/35">
            ...
            <label class="block">
              ...
              <input v-model="form.alt" type="text" placeholder="未入力の場合はタイトルを使用します。" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
              <p class="mt-2 text-xs leading-5 text-white/35">
            ...
            <div class="block">
              ...
              <ul v-if="currentColors.length" class="mt-3 flex flex-wrap gap-2">
                ...
                >
                  <span class="h-3.5 w-3.5 rounded-full border border-white/30" :style="{ backgroundColor: color }"></span>
                  <span>{{ color }}</span>
                  <button
                    type="button"
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
              <label class="block">
                ...
                <input type="file" accept="image/*" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white/70 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:font-bold file:text-slate-950" @change="setImageFile" />
              ...
              <label class="block">
                ...
                <input type="file" accept="image/*,.svg" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white/70 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:font-bold file:text-slate-950" @change="setLogoFile" />
            ...
            <label class="block">
              ...
              <input v-model="form.imageUrl" type="text" placeholder="Cloudinaryアップロード後は自動で設定されます。" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
              <p class="mt-2 text-xs leading-5 text-white/35">
            ...
            <label class="block">
              ...
              <input v-model="form.logoUrl" type="text" placeholder="任意。ロゴ画像URLを入力します。" class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none transition placeholder:text-white/25 focus:border-white/40" />
              <p class="mt-2 text-xs leading-5 text-white/35">
        ...
        <section>
          <div class="mb-5 flex items-center justify-between gap-4">
            ...
            <button type="button" class="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm font-bold text-white/60 transition hover:text-white" @click="loadDesignGraphics">
          ...
          <div v-else class="grid gap-5 md:grid-cols-2">
            <article
              ...
              :key="item.id"
              class="overflow-hidden rounded-[2rem] border border-white/10 bg-black/30 shadow-[0_18px_50px_rgba(0,0,0,0.28)]"
            >
              <div class="relative h-52 bg-black/40">
                <img v-if="item.imageUrl" :src="designGraphicImageUrl(item.imageUrl)" :alt="item.alt" class="h-full w-full object-cover" />
                <div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                <div class="absolute left-5 top-5 flex gap-2">
                  <span class="rounded-full bg-black/60 px-3 py-1.5 text-xs font-bold text-white/70 backdrop-blur">
                    #{{ item.order }}
                  </span>
                  <span class="rounded-full bg-black/60 px-3 py-1.5 text-xs font-bold text-white/70 backdrop-blur">
                    {{ item.period || '期間未設定' }}
                ...
                <span class="absolute right-5 top-5 rounded-full px-3 py-1.5 text-xs font-black" :class="item.isVisible ? 'bg-emerald-300 text-emerald-950' : 'bg-white/15 text-white/50'">
                  {{ item.isVisible ? '公開' : '非公開' }}
                </span>
                <div class="absolute bottom-5 left-5 right-5">
                  <img v-if="item.logoUrl" :src="designGraphicImageUrl(item.logoUrl)" :alt="`${item.title} logo`" class="mb-3 max-h-10 max-w-36 object-contain" />
              ...
              <div class="p-5">
                <div class="mb-3 flex flex-wrap items-center gap-2 text-xs font-bold text-[rgb(255,255,130)]/80">
                  <span>{{ item.category }}</span>
                  <span v-if="item.isFeatured" class="rounded-full bg-[rgb(255,255,130)] px-2 py-1 text-slate-950">注目</span>
                </div>
                <p class="line-clamp-2 text-sm leading-6 text-white/55">
                  {{ item.description }}
                </p>
                <ul class="mt-4 flex flex-wrap gap-2">
                  <li v-for="color in item.colors" :key="`${item.id}-${color}`" class="flex items-center gap-2 rounded-full bg-white/5 px-3 py-1.5 text-xs font-bold text-white/60">
                    <span class="h-3 w-3 rounded-full border border-white/30" :style="{ backgroundColor: color }"></span>
                    {{ color }}
                  </li>
                </ul>

                <div class="mt-5 flex gap-2">
                  <button type="button" class="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-black text-slate-950 transition hover:bg-[rgb(255,255,130)]" @click="editDesignGraphic(item)">
                  ...
                  <button type="button" class="inline-flex items-center justify-center rounded-full border border-red-300/20 px-4 py-2 text-red-200 transition hover:bg-red-500/10" @click="deleteDesignGraphic(item.id)">
